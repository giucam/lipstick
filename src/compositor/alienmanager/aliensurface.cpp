...
AlienSurface *AlienSurface::fromResource(::wl_resource *resource)
{
    return static_cast<AlienSurface *>(Resource::fromResource(resource)->alien_surface_object);
}
...
void AlienSurface::alien_surface_destroy_resource(Resource *)
{
    if (!m_deleting) {
        m_deleting = true;
        delete this;
...
void AlienSurface::alien_surface_set_transform_mask(Resource *resource, uint32_t transform)
{
    Q_UNUSED(resource)
    Q_UNUSED(transform)
}
...
void AlienSurface::alien_surface_request_state(Resource *resource, wl_array *array)
{
    Q_UNUSED(resource);
